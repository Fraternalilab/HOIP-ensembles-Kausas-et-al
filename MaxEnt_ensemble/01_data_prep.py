...
def format_fits(path_to_fits, maxq, output_dir):
	...
	fits.sort()
	exp_scattering = read_crysol_fit(fits[0], maxq)[5:, :3]
	experimental_data_prep(exp_scattering)
	q = exp_scattering[:, :1]
	for fit in fits:
		fit_idx = re.findall(r"\d+", os.path.basename(fit))[0][:-2]
		theoretical_scattering = read_crysol_fit(fit, maxq)[5:, 3:]
...
format_fits(path_to_fits="crysol_fits/*.fit", maxq=0.27, output_dir="./input.files/simulated_data/")
